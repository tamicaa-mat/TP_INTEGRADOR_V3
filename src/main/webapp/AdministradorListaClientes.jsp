		...
		<div class="d-flex justify-content-between align-items-center mb-4">
			<div>
				<span class="fw-bold me-2">Ver:</span> <a
					href="ClienteServlet?filtro=activos">Activos</a> <a
				...
				<tbody>
					<c:forEach var="cliente" items="${listaClientes}">
						<tr>
							...
							<td><c:choose>
									<c:when
										test="${not empty cliente.usuario and not empty cliente.usuario.nombreUsuario}">
							...
							<td><c:choose>
									<c:when test="${cliente.estado}">
										...
										<c:if
											test="${empty cliente.usuario or empty cliente.usuario.nombreUsuario}">
											<a
												href="UsuarioServlet?action=mostrarFormularioAlta&dniCliente=${cliente.dni}">Crear
												Acceso</a>
										</c:if>
									</c:when>
									<c:otherwise>
