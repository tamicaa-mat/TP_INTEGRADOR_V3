				...
				<tbody>
					<c:forEach var="usuario" items="${listaUsuarios}">
						<tr>
							...
							<td><c:out
									value="${usuario.getTipoUsuario().getDescripcion()}" /></td>
							<td><c:choose>
							...
							<td>
								<%-- No se puede modificar al usuario admin --%> <c:if
									test="${usuario.getTipoUsuario().getDescripcion() != 'Administrador'}">
									<c:choose>
										<c:when test="${usuario.isEstado()}">
											<a
										...
										<c:otherwise>
											<a
												href="UsuarioServlet?action=cambiarEstado&id=${usuario.getIdUsuario()}&estado=false"
												class="btn btn-success btn-sm">Activar</a>
										</c:otherwise>
									</c:choose>
									<a
										href="UsuarioServlet?action=resetearPassword&id=${usuario.getIdUsuario()}"
										class="btn btn-secondary btn-sm"
										onclick="return confirm('¿Está seguro que desea resetear la contraseña de este usuario?');">Resetear
										Pass</a>
								</c:if>
							</td>
						</tr>
