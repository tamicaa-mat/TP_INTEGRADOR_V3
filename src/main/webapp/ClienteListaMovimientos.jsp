...
<body class="bg-light">
	...
	<main class="container py-5">
		...
		<div class="container mb-4">
			<form method="get" action="MovimientoServlet"
				class="row g-3 align-items-end">
				<div class="col-md-4">
					<label for="cuenta" class="form-label">Cuenta</label> <select
						name="cuenta" id="cuenta" class="form-select">
						<option value="0" ${filtroCuenta == 0 ? "selected" : ""}>Todas</option>
						<c:forEach var="c" items="${cuentas}">
							<c:choose>
								<c:when test="${not empty c.tipoCuenta}">
									<option value="${c.idCuenta}"
										${filtroCuenta == c.idCuenta ? "selected" : ""}>
										${c.tipoCuentaObjeto.descripcion} Nº ${c.numeroCuenta}</option>
								</c:when>
								<c:otherwise>
									<option disabled>Error: cuenta sin tipo
										(${c.idCuenta})</option>
								</c:otherwise>
							</c:choose>
						</c:forEach>
				...
				<div class="col-md-4">
					<label for="tipo" class="form-label">Tipo de Movimiento</label> <select
						name="tipo" id="tipo" class="form-select">
						<option value="0" ${filtroTipo == 0 ? "selected" : ""}>Todos</option>
						<option value="1" ${filtroTipo == 1 ? "selected" : ""}>Alta
							Cuenta</option>
						<option value="2" ${filtroTipo == 2 ? "selected" : ""}>Alta
							Prestamo</option>
						<option value="3" ${filtroTipo == 3 ? "selected" : ""}>Pago
							Prestamo</option>
		...
		<div class="table-responsive mb-5">
			<table id="tablaFiltrados"
				class="table table-bordered table-hover align-middle text-center display">
						...
						<c:when test="${empty movimientos}">
							<tr>
								<td colspan="4" class="text-center text-muted">No se
									encontraron movimientos para los filtros seleccionados.</td>
							</tr>
						</c:when>
						<c:otherwise>
