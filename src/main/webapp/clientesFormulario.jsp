<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import="java.util.ArrayList, dominio.Cliente, dominio.Provincia, dominio.Localidad, dominio.Usuario"%>
...
ArrayList<Localidad> listaLocalidadesFiltradas = (ArrayList<Localidad>) request
		.getAttribute("listaLocalidadesFiltradas");
...
<body class="bg-light">
	...
	<main
				...
				<div class="card-body">
					...
					<form action="ClienteServlet" method="post" id="formCliente">
						...
						<input type="hidden" name="action" value="modificar" /> <input
							type="hidden" name="txtDni" value="<%=clienteAEditar.getDni()%>" />
						<%
						} else {
						%>
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control"
								value="DNI: <%=clienteAEditar.getDni()%>" disabled>
							<%
							} else {
							%>
							<input type="text" class="form-control" name="txtDni"
								placeholder="DNI"
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getDni()
		: (request.getParameter("txtDni") != null ? request.getParameter("txtDni") : "")%>"
						...
						<div class="mb-3">
							<input type="text" class="form-control" name="txtCuil"
								placeholder="CUIL"
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getCuil()
		: (request.getParameter("txtCuil") != null ? request.getParameter("txtCuil") : "")%>"
								required>
						</div>
						...
						<div class="mb-3">
							<input type="text" class="form-control" name="txtNombre"
								placeholder="Nombre"
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getNombre()
		: (request.getParameter("txtNombre") != null ? request.getParameter("txtNombre") : "")%>"
						...
						<div class="mb-3">
							<input type="text" class="form-control" name="txtApellido"
								...
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getApellido()
		: (request.getParameter("txtApellido") != null ? request.getParameter("txtApellido") : "")%>"
						...
						<div class="mb-3">
							<select class="form-select" name="ddlSexo" required>
								<%
								String sexoSeleccionado = clienteParaFormulario != null
										? clienteParaFormulario.getSexo()
										: request.getParameter("ddlSexo");
								%>
								<option value="">Seleccione sexo</option>
								<option value="M"
									<%="M".equals(sexoSeleccionado) ? "selected" : ""%>>Masculino</option>
								<option value="F"
									<%="F".equals(sexoSeleccionado) ? "selected" : ""%>>Femenino</option>
							</select>
						</div>

						<div class="mb-3">
							<input type="date" class="form-control" name="txtFechaNacimiento"
								value="<%=clienteParaFormulario != null && clienteParaFormulario.getFechaNacimiento() != null
		? clienteParaFormulario.getFechaNacimiento()
		: (request.getParameter("txtFechaNacimiento") != null ? request.getParameter("txtFechaNacimiento") : "")%>"
						...
						<div class="mb-3">
							<input type="text" class="form-control" name="txtDireccion"
								...
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getDireccion()
		: (request.getParameter("txtDireccion") != null ? request.getParameter("txtDireccion") : "")%>"
						...
						<div class="mb-3">
							<select class="form-select" name="ddlProvincia" id="ddlProvincia"
								...
								if (listaProvincias != null) {
									...
									for (Provincia prov : listaProvincias) {
										String selected = "";
										if (provinciaSeleccionada != null && provinciaSeleccionada.equals(String.valueOf(prov.getIdProvincia()))) {
									selected = "selected";
										}
										out.println("<option value='" + prov.getIdProvincia() + "' " + selected + ">" + prov.getDescripcion()
										+ "</option>");
									}
								}
								%>
						...
						<div class="mb-3">
							<select class="form-select" name="ddlLocalidad" id="ddlLocalidad"
								required>
								<%
								...
								if (localidadesAMostrar != null) {
									...
									for (Localidad loc : localidadesAMostrar) {
										String selected = "";
										if (localidadSeleccionada != null && localidadSeleccionada.equals(String.valueOf(loc.getIdLocalidad()))) {
									selected = "selected";
										}
										out.println(
										"<option value='" + loc.getIdLocalidad() + "' " + selected + ">" + loc.getDescripcion() + "</option>");
									}
								}
								}
								%>
						...
						<div class="mb-3">
							<input type="text" class="form-control" name="txtNacionalidad"
								...
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getNacionalidad()
		: (request.getParameter("txtNacionalidad") != null ? request.getParameter("txtNacionalidad") : "")%>"
								required>
						</div>
						...
						<div class="mb-3">
							<input type="email" class="form-control" name="txtEmail"
								...
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getCorreoElectronico()
		: (request.getParameter("txtEmail") != null ? request.getParameter("txtEmail") : "")%>"
						...
						<div class="mb-3">
							<input type="text" class="form-control" name="txtTelefono"
								...
								value="<%=clienteParaFormulario != null
		? clienteParaFormulario.getTelefono()
		: (request.getParameter("txtTelefono") != null ? request.getParameter("txtTelefono") : "")%>"
						...
						<div class="d-flex justify-content-center gap-2">
							...
							<button type="button" class="btn btn-outline-danger btn-sm w-50"
								onclick="limpiarDatos()">Empezar de Nuevo</button>
							<%
							}
							%>
						</div>
