	...
	<main class="container py-5">
		<div class="d-flex justify-content-between align-items-center mb-4">
			...
			<button onclick="window.print();"
				class="btn btn-secondary btn-sm no-print">Imprimir
				Directorio</button>
		</div>
				...
				<tbody>
					<c:forEach var="cliente" items="${listaClientes}">
						<tr>
							...
							<td><c:out
									value="${cliente.getNombre()} ${cliente.getApellido()}" /></td>
							...
							<td><c:out
									value="${cliente.getLocalidad().getDescripcion()}" /></td>
							<td><c:out
									value="${cliente.getLocalidad().getProvincia().getDescripcion()}" /></td>
							<td><c:choose>
