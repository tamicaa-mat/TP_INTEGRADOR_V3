	...
	<main class="container py-5">
		...
		<div class="table-responsive">
			<table id="tablaPrestamos"
				class="display table table-bordered table-hover align-middle text-center">
					...
					<c:forEach var="p" items="${prestamos}">
						<tr>
							...
							<td><c:choose>
									...
									<c:when test="${p.estado == 1 && p.cantidadCuotas > 0}">
										<span class="text-info fw-bold">En Pago</span>
									</c:when>
									<c:when test="${p.estado == 1 && p.cantidadCuotas == 0}">
							...
							<td>
								<!-- Solo mostrar botones si está pendiente (Estado = 0) --> <c:if
									test="${p.estado == 0}">
								...
								</c:if> <!-- Mostrar mensaje informativo para otros estados --> <c:if
									test="${p.estado == 1 && p.cantidadCuotas > 0}">
									<small class="text-muted">En proceso de pago</small>
								</c:if> <c:if test="${p.estado == 1 && p.cantidadCuotas == 0}">
									<small class="text-success">✅ Completado</small>
								</c:if> <c:if test="${p.estado == 2}">
									<small class="text-danger">❌ Denegado</small>
								</c:if>
							</td>
						</tr>
					</c:forEach>
