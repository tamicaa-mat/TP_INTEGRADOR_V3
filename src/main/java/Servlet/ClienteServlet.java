			...
			try {
				...
				if (email != null && !email.trim().isEmpty() && clienteNegocio.existeEmail(email.trim())) {
					throw new DatosInvalidosException(
							"El correo electrónico '" + email.trim() + "' ya está registrado en el sistema");
				}
				...
				if (edad < 18) {
					throw new EdadInvalidaException(
							"El cliente debe tener al menos 18 años. Edad actual: " + edad + " años.");
				}
				...
				response.sendRedirect(
						request.getContextPath() + "/UsuarioServlet?action=mostrarFormularioAlta&dniCliente=" + dni);
